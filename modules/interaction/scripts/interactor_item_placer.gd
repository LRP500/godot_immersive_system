...
func _create_ghost(item: Node3D) -> Node3D:
    var ghost: Node3D = item.duplicate()
    Node3dHelpers.disable(ghost)
    var meshes := Node3dHelpers.get_children_of_type(ghost, "GeometryInstance3D")
    for mesh in meshes:
