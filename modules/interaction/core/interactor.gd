...
func _process_interactions(_interactions: Array[Interaction]) -> void:
    for interaction in _interactions:
        if !interaction.is_enabled:
            continue
        if InputActionMapManager.is_action_just_pressed(interaction.input_map_action):
        ...
        if InputActionMapManager.is_action_just_released(interaction.input_map_action):
